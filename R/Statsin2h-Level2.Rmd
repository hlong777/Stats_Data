...
summarise(_ _ _, mean_Rating = mean(_ _ _), std_Rating = sd(_ _ _), variance = var(_ _ _), count = length(_ _ _) )
```
### Visual summary
...
ggplot(d_ _ _) + geom_boxplot(aes(x = _ _ _, y = _ _ _))
```
...
{
  ind_A <-  sample.int(length(total), size = _ _ _, replace = TRUE)
  ind_B <-  sample.int(length(total), size = _ _ _, replace = TRUE)
